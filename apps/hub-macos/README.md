...
Web 双向通话已有短时实测，**新的原生 AudioQueue 通道尚需单独真机验证**。客户端支持已验证 QDC507 的旧式 ADB 首次授权，修改前保存配置备份，只开启 ADB，重启后核对设备；不撤销 root、不刷固件。未知授权协议或被其他服务占用时停止。没有模块声卡时，不会假装通话音频成功。详见 [初始化方案](../../docs/QDC507_INITIALIZATION.md)。
